// =====================================================================
// Jordan Monopoly — static board data (CLIENT copy)
// =====================================================================
//
// This is the single source of truth for the board layout, prices, rent
// tables, and the Chance / Community Chest decks. The Edge Function keeps a
// byte-identical MIRROR at supabase/functions/_shared/monopolyBoard.ts (Deno
// cannot import from src/, same precedent as checkersRules.js ⇄ checkers-move).
// A Deno parity test (monopolyBoard.test.ts) deep-compares the two so they can
// never silently drift. The SERVER copy is authoritative for money; this copy
// is for display + instant client-side affordance hints only.
//
// Tile shape:
//   { i, type, name:{en,ar}, color?, group?, price?, rent?, house?, mortgage?, tax? }
//   type ∈ go | property | railroad | utility | tax | chance | chest | jail |
//          free_parking | go_to_jail
//   rent (property only) = [base, 1house, 2house, 3house, 4house, hotel]
//
// All money is JOD. The structure mirrors the classic 40-tile board exactly
// (prices, rents, positions) so the game plays identically; only the names are
// Jordan-themed.
...
export const MORTGAGE_INTEREST = 0.1 // unmortgage costs mortgage × 1.10
export const BANKRUPTCY_FEE = 0.1 // 10% transfer fee on mortgaged value to a creditor

// 8 token keys (rendered as colored dots; lucide/emoji map lives in the UI).
export const TOKENS = ['car', 'ship', 'thimble', 'dog', 'hat', 'boot', 'iron', 'wheelbarrow']

// Color groups: display hex + how many tiles complete the set (monopoly).
...
// ---------- card decks ----------
// effect kinds (engine-interpreted):
//   move_to {tile, collectGo}                 — advance to a tile (credit GO if passed when collectGo)
//   move_rel {steps}                          — move relative (may be negative)
//   move_to_nearest {kind, rentMult?}         — nearest 'railroad' or 'utility' (rentMult on landing)
//   collect {amount} / pay {amount}           — flat bank credit / debit
//   collect_from_each {amount} / pay_each {amount}
//   goto_jail                                 — straight to jail, no GO
//   goojf                                     — keep a Get-Out-Of-Jail-Free card
//   repairs {perHouse, perHotel}              — pay the bank for each building owned
...
// ---------- derived helpers (shared by client + server) ----------

// Tile indices that can be owned (properties, railroads, utilities) — the 28 rows
// monopoly_properties is seeded with at game start.
export const OWNABLE = BOARD.filter((t) => t.type === 'property' || t.type === 'railroad' || t.type === 'utility').map((t) => t.i)

export const tileAt = (i) => BOARD[((i % 40) + 40) % 40]
...
// Self-check (also asserted by the Deno parity test on the server copy).
